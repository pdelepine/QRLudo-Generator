...
interface SGTextNode extends SGNode {
  url: string;
  description: string;
  btn_save_modification: p5.Element | null;
  btn_add_audio: p5.Element | null;
  btn_delete_audio: p5.Element | null;
  btn_discard_modification: p5.Element | null;
  containError: boolean;
}
/**
 * Cette classe représente un Noeud de question dans l'interface du serious game fait avec p5.js
 * */
class SGTextNode extends SGNode implements SGTextNode {
  /**
  ...
  displayQuestionZone(myP5: MyP5) {
    ...
    this.btn_add_audio.mousePressed(() => {
      SGTextNode.addAudio(self, myP5);
    });
    ...
    this.btn_delete_audio.mousePressed(() => {
      SGTextNode.deleteAudio(self, myP5);
    });
    ...
    this.btn_discard_modification.mousePressed(() => {
      SGTextNode.discardModification(self);
    });
    this.btn_discard_modification.class(
      "btn btn-outline-success btn-unique-xl"
    );
    ...
    this.btn_save_modification.mousePressed(() => {
      SGTextNode.saveModification(self, myP5);
    });
  ...
  /** Add an audio file */
  static addAudio(self, myP5: MyP5) {
    myP5.setLastNodeClickedType("text");
  }

  /** Delete an audio file */
  static deleteAudio(self, myP5: MyP5) {
    myP5.setLastNodeClickedType("text");
    self.url = "";
    supprimerChampSon();
  }
  ...
  static saveModification(self, myP5: MyP5) {
  ...
  display(myP5: MyP5) {
    ...
    myP5.text(
      this.name,
      this.x + this.w / 2 - myP5.textWidth(this.name) / 2,
      this.y + this.h / 1.8
    );
    myP5.pop();
  }
